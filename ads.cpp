...
class Flight // class for managing the availabe functions of a flight
{
	private:
		...
		int flightNo, seats, booked_seats;
		string from, to;
...
void Menu::displayMenu(){
	...
	do{
			...
			cout << "2. Display passenger personal info \n";
			cout << "3. Book reservation \n";
...
void Flight::addFlight(){
	...
	/* --DEPARTURE-- */
	cout << "Departure City(no space between words): ";
	flag = false;
	...
	/* --DESTINATION-- */
	cout << "Destination City(no space between words): ";
	flag = false;
...
bool Person::displayPersonInfo(int passport){
	...
	for (std::list<Person>::iterator i = plist.begin(); i != plist.end(); ++i){
		...
		if(passport == i -> passportNo){
			...
			cout << left << setw(15) << "NAME" << left << setw(15) << "SURNAME" << left << setw(15) << "PASSPORT_No" <<  endl;

			cout << left << setw(15) << i -> name << left << setw(15) << i -> surname << left << setw(15) << i -> passportNo <<  endl << endl;
			cout << "Your flight(s): ";
